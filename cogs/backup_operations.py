    ...
    async def create_alliance_backup_zip(self, alliance_id: int, guild_id: int):
        ...
        with sqlite3.connect(database_path(USERS_DB, 'users.sqlite')) as udb:
            ...
            ucursor.execute("""
                ...
                ORDER BY fid
            """, (alliance_id,))
            users_rows = ucursor.fetchall()

        alliance_data = dict(alliance_row)
        users_data = [dict(row) for row in users_rows]

        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        safe_name = "".join(c if c.isalnum() or c in ("-", "_") else "_" for c in alliance_data.get("name", f"alliance_{alliance_id}"))
        zip_path = os.path.join(tempfile.gettempdir(), f"{safe_name}_{alliance_id}_{timestamp}_backup.zip")
        ...
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zf:
            zf.writestr("alliance.json", json.dumps(export_payload, indent=2, default=str))
            zf.writestr("members.json", json.dumps(users_data, indent=2, default=str))
...
class AllianceSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        selected_label = self.alliance_map.get(str(selected_id), str(selected_id))

        backup_cog = interaction.client.get_cog("BackupOperations")
        if not backup_cog:
        ...
        try:
            zip_path, alliance_name, member_count = await backup_cog.create_alliance_backup_zip(
                selected_id,
            ...
            file = discord.File(
                ...
                description=f"Backup export for {alliance_name}"
            )

            await interaction.followup.send(
                content=f"✅ Backup ready for **{alliance_name}** (`{selected_id}`) with {member_count} members.",
                file=file,
                ephemeral=True
            )
        except Exception as e:
            await interaction.followup.send(
                content=f"❌ Failed to create backup for **{selected_label}**: `{e}`",
                ephemeral=True
            )
